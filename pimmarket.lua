...
-- ========== ОТРИСОВКА СТРОКИ ==========
local function drawSingleRow(y, item, isSelected, isHovered)
...
local function drawScrollBar()
  ...
  local barHeight = math.max(3, math.floor(visibleRows * visibleRows / total))
  local maxPos = visibleRows - barHeight
  local barPos = math.floor((listScroll - 1) * maxPos / (total - visibleRows)) + 1
...
local function drawBuyItemsList()
  ...
  for i = 1, visibleRows do
    ...
    drawSingleRow(y, item, isSelected, isHovered)
  end
...
local function drawHelpScreen()
  ...
  elseif helpPage == 2 then
    ...
    gpu.setForeground(0x00ff88) gpu.set(4, 6, "$ - Только ресурсы")
    gpu.setForeground(0x00aaff) gpu.set(4, 7, "♦ - Только эмеральны")
    gpu.setForeground(0xffaa00) gpu.set(4, 8, "$ и ♦ - Смежная оплата")
    gpu.setForeground(0xff7300)
...
-- (Остальные функции drawAccount, drawAccountLoading, retryAccountAfterTokenRefresh и т.д. оставлены без изменений)
    ...
    if ev[1] == "modem_message" then
      ...
      if sender == serverAddress then
        ...
        if success and msg and msg.op == "welcome" and msg.token then
          ...
          drawAccountLoading()
          modem.send(serverAddress, 0xffef, serialization.serialize({op = "getAccount", name = currentPlayer, token = currentToken}))
          return
        end
      end
...
while true do
  ...
  if e == "touch" then
    ...
    if currentScreen == "menu" then
      ...
      if y == 23 and x >= 4 and x <= 13 then goToHelp() end
    ...
    elseif currentScreen == "help" then
      ...
      if y == 20 then
        if x >= pageX and x < pageX + 4 and helpPage > 1 then
          helpPage = helpPage - 1
          drawHelpScreen()
        elseif x >= pageX + unicode.len(pageStr) - 4 and helpPage < HELP_PAGES then
          helpPage = helpPage + 1
          drawHelpScreen()
        end
      end
  ...
  elseif e == "drag" and currentScreen == "shop_buy" then
    local x, y = ev[3], ev[4]
    if y >= 6 and y <= 17 and x >= 2 and x <= 77 then
      local rel = y - 5
  ...
  if e=="player_on" or e=="pim" or e=="pim_player_enter" then
    ...
    -- (остальная логика авторизации без изменений)
