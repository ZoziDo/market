...
local authTimer = nil            -- хранит id таймера, чтобы можно было отменить

-- ========== НАСТРОЙКА ЭКРАНА ==========
gpu.setResolution(80, 25)          -- весь монитор
...
-- Функция завершения авторизации (вызывается таймером)
...
while true do
  ...
  elseif e == "player_on" then
    ...
    if currentScreen ~= "auth" then
      ...
      if authTimer then event.cancel(authTimer) end
      authTimer = event.timer(3, finishAuth)
    end
  elseif e == "player_off" then
    ...
    drawWelcomeScreen()
    -- Отменяем таймер авторизации, если был
    if authTimer then event.cancel(authTimer); authTimer = nil end
