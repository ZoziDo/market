-- Standalone OCIF6 viewer for OpenComputers / OpenOS
...
local function drawOCIF6(path)
  ...
  if sig ~= "OCIF" then
    file:close()
    error("Not an OCIF file")
  end

  local method = readByte(file)
  if method ~= 6 then
    file:close()
    error("This viewer supports OCIF method 6 only. Got: " .. tostring(method))
  end
  ...
  local info = "Viewing: " .. path
  gpu.set(1, math.min(sh, oy + height + 1), info:sub(1, sw))
end
